{
 ...
 TCanvas *scatola = new TCanvas("scatola","",650,500);
   ...
   scatola->SetTickx(1);
   scatola->SetTicky(1);
  TGraphErrors *dati = new TGraphErrors("moduloyang.dat");
...
dati->SetTitle("");
 ...
 //distanza del titolo dell'asse dall'asse//
  ...
  //**************************************//
     dati->GetXaxis()->SetTitle("pesi (grammi)");
	 dati->GetYaxis()->SetTitle("allungamenti (mm)");
  dati->SetMarkerStyle(20);
  dati->GetYaxis()->SetRangeUser(-1,10);

  dati->Draw("ape");
  TF1 * modello = new TF1("modello","[0]+[1]*x",200,200);
  modello->SetLineColor(2); // cambio il colore di default della classe impostando a mano il colore rosso
  dati->Fit("modello","R");//fittare nel dominio di definizione del modello
TLatex *pino = new TLatex(200, 7 ,Form("intercetta = %5.3f #pm  %5.3f",modello->GetParameter(0), modello->GetParError(0)));
    pino->SetTextFont(132);
    pino->SetTextSize(0.05);
    pino->Draw();
    TLatex *pina = new TLatex(200, 5,Form("pendenza = %5.3f #pm  %6.4f",modello->GetParameter(1), modello->GetParError(1)));
  ...
  scatola->SaveAs("Figyangfit.pdf");
  scatola->SaveAs("Figyangfit.jpeg");
