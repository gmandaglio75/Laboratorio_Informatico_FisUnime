...
float max, min;				//calcolo del massimo e minimo tramite un confronto tra gli elementi dei vettori max e min
max=misure[0];
min=misure[0];
...
cin>>nint;
float l=(max-min)/(float)nint;				//lunghezza intervalli		
...
if((misure[v]>=min+i*l)&&(misure[v]<=min+(i+1)*l))      //per ogni intervallo conta quali valori cadono entro l'intervallo stesso
...
cout<<min+l/2+n*l<<"\t"<<bin[n]<<endl;                //stampa istogramma e valori medi di ogni intervallino
